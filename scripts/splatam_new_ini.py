...
def initialize_new_params(new_pt_cld, mean3_sq_dist, gaussian_distribution, 
    ...
    if curr_data['id'] %150 == 0 and curr_data['id'] >= 500:
        ...
        filtered_ps_means = b_pcd
        filtered_rgb_colors = ps_gaussians.harmonics.squeeze(0)[..., 0].to(device="cuda")
        filtered_logit_opacities = torch.logit(ps_gaussians.opacities.squeeze(0).unsqueeze(-1)).to(device="cuda")
        
        filtered_log_scales = torch.log(torch.sqrt(ps_gaussians.covariances.squeeze(0).diagonal(dim1=-2, dim2=-1).mean(dim=-1, keepdim=True)))

        unnorm_rots = np.tile([1, 0, 0, 0], (filtered_ps_means.shape[0], 1))
        params = {
...
def add_new_gaussians(params, variables, curr_data, sil_thres, 
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        if time_idx % 150 == 0 and time_idx >= 500:
            last_cam_rot = torch.nn.functional.normalize(params['cam_unnorm_rots'][..., time_idx-50].detach())
            last_cam_tran = params['cam_trans'][..., time_idx-50].detach()
            last_w2c = torch.eye(4).cuda().float()
            last_w2c[:3, :3] = build_rotation(last_cam_rot)
            last_w2c[:3, 3] = last_cam_tran
...
def rgbd_slam(config: dict, pixelsplat):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        color, depth, _, gt_pose = dataset[time_idx]
        if (time_idx) %150 == 0 and time_idx >= 500:
            last_color, _, _, last_gt_pose = dataset[time_idx-50]
